...
public class MobInfoService : IDisposable, IMobInfoService
{
    ...
    private void LoadDeepDungeonMobInfos()
    {
        ...
        try
        {
            LoadMobInfoFile(Path.Combine(NecroLensDI.PluginInterface.AssemblyLocation.Directory?.FullName!, "data/allMobs.json"));
        }
        catch (Exception e)
        {
        ...
        if (MobInfoDictionary.Count <= 0)
        {
            try
            {
                ...
                const string uri =
                    "https://raw.githubusercontent.com/Jukkales/NecroLensDI/main/NecroLensDI/Data/allMobs.json";
